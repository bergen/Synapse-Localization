...
def masked_mse(outs, y,  mask=None):
    assert mask.shape == outs.shape, f"Mask shape {mask.shape} doesn't match output shape {outs.shape}"
    
    # Apply mask and compute MSE
    outs_masked = torch.masked_select(outs, mask)
    y_masked = torch.masked_select(y, mask)

    if outs_masked.numel() == 0:
        # Return a zero loss that is still connected to the graph.
        # Multiplying an element from outs by 0.0 ensures the loss is linked to the model's outputs.
        return (outs.sum() * 0.0)
    
    return F.mse_loss(outs_masked, y_masked)

def masked_huber(outs, y,  mask=None, huber_delta=0.5):
    assert mask.shape == outs.shape, f"Mask shape {mask.shape} doesn't match output shape {outs.shape}"
    ...
    return nn.HuberLoss(delta=huber_delta)(outs_masked, y_masked)
